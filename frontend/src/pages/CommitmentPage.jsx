...
export default function CommitmentPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Could not create commitment');
      setCommitments((prev) => [data.commitment, ...prev]);
      setForm({ title: '', description: '', stake_amount: '25', reward_amount: '50', target_date: '' });
      setMessage('Commitment created. Track it here and complete it when ready.');
    } catch (error) {
  ...
  const completeCommitment = async (commitmentId) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to complete commitment');
      setCommitments((prev) => prev.map((item) => item.id === commitmentId ? { ...item, status: 'completed', completed_at: data.completed_at } : item));
      setMessage('Commitment marked complete — reward recorded.');
    } catch (error) {
      setMessage(error.message);
    } finally {
      setLoading(false);
        ...
        <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
          <section className="space-y-6 rounded-3xl border border-slate-800 bg-slate-900 p-8">
            <div className="flex items-center gap-3 text-indigo-300">
              <Target size={20} />
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Commitment Vault</p>
                <h1 className="text-3xl font-bold text-white">Stake goals and earn the upside</h1>
              </div>
            </div>
            <p className="text-slate-400 leading-relaxed">Create a behavior commitment, lock in a reward, and track completion. Your success becomes a product feature and revenue source.</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="block text-sm text-slate-300">
                  ...
                  <input
                    ...
                    placeholder="Launch a site this week"
                    required
                  />
                </label>
                <label className="block text-sm text-slate-300">
              ...
              <label className="block text-sm text-slate-300">
                ...
                <textarea
                  ...
                  placeholder="Create a billboard landing page and publish it live."
                  required
                />
              </label>
          ...
          <section className="space-y-4">
            <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-semibold text-white">Your commitments</h2>
              </div>
              <p className="mt-2 text-slate-400 text-sm">Track, complete, and reward successful behavior-based goals.</p>
            </div>

            <div className="space-y-4">
              {commitments.length === 0 ? (
                <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 text-slate-400">No commitments yet. Create one from the form.</div>
              ) : commitments.map((commitment) => (
                <div key={commitment.id} className="rounded-3xl border border-slate-800 bg-slate-900 p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="text-sm text-slate-500 uppercase tracking-[0.2em]">{commitment.status}</div>
                  ...
                  <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-sm text-slate-400">
                    <span>Reward ${commitment.reward_amount}</span>
                    <span>Target {commitment.target_date || 'TBD'}</span>
